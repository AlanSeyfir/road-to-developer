-- Some comments are the answers of the exercises, maybe they're the same or different
...
-- [DONE] 2. Select only the Names of all the Bands
SELECT name AS Band_Name FROM bands;
-- SELECT bands.name AS 'Band Name' FROM bands;

-- [DONE] 3. Select the Oldest Album
SELECT * FROM albums WHERE release_year = 1988;
...
-- [DONE] 4. Get all Bands that have Albums
SELECT DISTINCT bands.name AS 'Band name' FROM bands
JOIN albums ON bands.id = albums.band_id;
-- Same answer lmao

-- [DONE] 5. Get all Bands that have No Albums
SELECT DISTINCT bands.name AS 'Band Name' FROM bands
